...
//获取文章列表
func getArtilceList(baseUrl, cateUrl, siteName string, cateId int) (data []Article, err error) {
	doc, err := htmlquery.LoadURL(cateUrl)
	if err != nil {
		return nil, err
	}
	list := htmlquery.Find(doc, "//div[@class='blogs-list']/ul/li")
	var artilceList = []Article{}
	for _, item := range list {
		...
		artilceList = append(artilceList, article)
	}
	db.Create(&artilceList)
...
func getPageRule(CateUrl string) (pre string, count int, err error) {
	...
	page := htmlquery.Find(doc, "//div[@class='whitebg paihang']/ul/li/a")
	...
	stringTotal := strings.Split(arr2[0], ".")
...
func spider(baseUrl, name string, cate ArticleCate, count int, pageRule string) {
	if count > 0 {
		for page := 1; page <= count; page++ {
			...
			list, err := getArtilceList(baseUrl, articleListUrl, name, cate.Id)
			if err != nil {
				panic(err)
			}
			fmt.Println("列表：", list)
